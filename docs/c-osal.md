...
`pkgs/c` is portable C: its public headers and sources do not include a
...
`pkgs/c/include/pixa_osal.h` covers the small filesystem surface the core
...
`pkgs/c/tests/cgo` is the portable C test package. It compiles the core C
