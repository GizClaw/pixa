...
`packages/c/tests/cgo` proves the boundary with a real Go-backed provider. Its
thin C bridge forwards the ABI to Go `os` and `io` filesystem operations, so
the same test runs natively on Linux, macOS, and Windows. Run it with
`make c-check`; CI runs it on all three hosts.
